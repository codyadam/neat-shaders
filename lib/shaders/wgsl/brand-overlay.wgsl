...
fn label_cov(p: vec2f, center: vec2f, radius: f32) -> f32 {
  ...
  let comma_w = px * 0.4;
  acc = max(acc, glyph_cov(p, origin + vec2f(w, 0.0), vec2f(comma_w, px * 0.72), COMMA_IDX));
  acc = max(acc, int_cov(p, origin + vec2f(w + comma_w * 0.85, 0.0), px, y));
  return acc;
}
